...
const INSTRUCTORES_URI = "https://backend-core-proyecto.onrender.com/instructores";
const TEMAS_URI = "https://backend-core-proyecto.onrender.com/temas";
const SUBTEMATICAS_URI = "https://backend-core-proyecto.onrender.com/subtematicas"; // URI para subtemáticas
const UPDATE_COURSE_URI = "https://backend-core-proyecto.onrender.com/cursos";
...
const EditarCurso = ({ cursoId, onCancel, onRefresh }) => {
    const [curso, setCurso] = useState({
        email_instructor: "",
        id_tema: "",
        id_subtematica: "", // Agregado para la subtemática
        nombre: "",
        descripcion: ""
    });
    const [instructores, setInstructores] = useState([]);
    const [temas, setTemas] = useState([]);
    const [subtematicas, setSubtematicas] = useState([]); // Estado para subtemáticas
    const [error, setError] = useState("");
    ...
    useEffect(() => {
        ...
        const fetchSubtematicas = async () => {
            try {
                const res = await axios.get(SUBTEMATICAS_URI, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem("token")}`
                    }
                });
                setSubtematicas(res.data);
            } catch (error) {
                console.error("Error al obtener subtemáticas:", error);
                setError("No se pudieron obtener las subtemáticas.");
            }
        };
    ...
    return (
        <div className="editar-curso-container">
            ...
            <div className="form-group">
                <label>Subtemática</label>
                <select
                    name="id_subtematica"
                    value={curso.id_subtematica}
                    onChange={handleChange}
                    required
                >
                    <option value="">Seleccione una subtemática</option>
                    {subtematicas.map((subtematica) => (
                        <option key={subtematica.id_subtematica} value={subtematica.id_subtematica}>
                            {subtematica.tipo}
                        </option>
                    ))}
                </select>
